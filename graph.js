...
const width = 800;  // Tăng kích thước chiều rộng
const height = 600; // Tăng kích thước chiều cao
const nodeRadius = 20;

// Khởi tạo SVG
const svg = d3.select("#graphContainer")
    .append("svg")
    .attr("width", "100%")     // Sử dụng chiều rộng 100%
    .attr("height", "100%")    // Sử dụng chiều cao 100%
    .attr("viewBox", `0 0 ${width} ${height}`)  // Thêm viewBox để đảm bảo tỷ lệ
    .attr("preserveAspectRatio", "xMidYMid meet");  // Giữ tỷ lệ khung hình

// Thêm container cho zoom và pan
const container = svg.append("g")
    .attr("class", "container")
    .attr("transform", `translate(${width/2}, ${height/2})`);  // Căn giữa container

// Thêm zoom behavior
const zoom = d3.zoom()
    .scaleExtent([0.1, 4]) // Giới hạn tỷ lệ zoom từ 0.1 đến 4
    .on("zoom", (event) => {
...
function initializeGraph() {
    ...
    // Create nodes with better initial positions
    ...
    // Create edges (pentagon shape with cross connections)
    links = [
    ...
    addLogEntry("Đồ thị đã được khởi tạo - Chọn đỉnh bắt đầu và kết thúc");
...
// Thêm hàm generateNewGraph để khớp với onclick trong HTML
function generateNewGraph() {
    console.log('Generating new graph...');
    initializeGraph();
    addLogEntry("Đã tạo đồ thị mới");
} 